...
const formatFileName = (
  ...
  return `${cleanLang}-${cleanState}-${cleanTemple}-${cleanName}-${cleanContact}-${index}_${timestamp}_${randomStr}.${extension}`;
};
...
export default function OfferingForm({ userId }: { userId: string }) {
  ...
  formData.state && formData.state !== 'Other'
    ? [...(ISKCON_LOCATIONS[formData.state] || []), 'Other']
    : [];
    ...
    try {
      ...
      if (filesToUpload.length > 0) {
        for (let i = 0; i < filesToUpload.length; i++) {
          ...
          const finalCity =
            (formData.state === 'Other' || formData.city === 'Other')
              ? formData.customCity
              : formData.city;
                        ...
                        <>
                          <select
                            ...
                            onChange={e =>
                              setFormData({
                                ...formData,
                                city: e.target.value,
                                customCity: e.target.value === 'Other' ? '' : formData.customCity
                              })
                            }
                          >
                          ...
                          {formData.city === 'Other' && (
                            <input
                              ...
                              className="w-full mt-2 px-4 py-3 bg-stone-50 border border-stone-200 rounded-lg text-sm"
                            />
                          )}
                        </>
                      ) : (
