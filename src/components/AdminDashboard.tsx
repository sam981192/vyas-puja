...
const cleanFilenamePart = (text: string, toLowercase = false): string => {
  let val = (text || '').trim();
  if (toLowercase) {
    val = val.toLowerCase();
  }
...
const getDownloadFileName = (
  ...
  const cleanLang = cleanFilenamePart(language, true);
  const cleanState = cleanFilenamePart(state, true);
  const cleanTemple = cleanFilenamePart(city, true);
  const cleanName = cleanFilenamePart(name, false); // preserve Unicode devotee name
  const cleanContact = cleanFilenamePart(contact, false);

  const baseName = `${cleanLang}-${cleanState}-${cleanTemple}-${cleanName}-${cleanContact}`;
  if (totalFiles > 1) {
    return `${baseName}-${fileIndex}.${extension}`;
  }
...
const logRows = await fetchAllRows(
  supabase.from('file_statuses').select('*')
      ...
      (rows || []).forEach((row: any) => {
        ...
        group.fileIndices.push(row.file_index || 1);
      });
      ...
      const result = Array.from(grouped.values()).map((s: any) => {
        ...
        return { ...s, downloadUrl: s.fileUrls[0], allUrls: s.fileUrls, fileLogs, fileIndices: s.fileIndices };
      });
  ...
  const downloadSubmissionFiles = async (s: Submission) => {
    ...
    if (s.allUrls.length === 1) {
      const fileIndex = s.fileIndices?.[0] || 1;
      const fileName = getDownloadFileName(s.language, s.state, s.city, s.name, s.contact, fileIndex, 1, s.files[0]);
      await downloadFile(s.allUrls[0], fileName);
    } else {
      const zip = new JSZip();
      for (let i = 0; i < s.allUrls.length; i++) {
        try {
          ...
          const blob = await res.blob();
          const fileIndex = s.fileIndices?.[i] || (i + 1);
          const fileName = getDownloadFileName(s.language, s.state, s.city, s.name, s.contact, fileIndex, s.allUrls.length, s.files[i]);
          zip.file(fileName, blob);
        } catch { /* skip failed */ }
      }
      const content = await zip.generateAsync({ type: 'blob' });
      const cleanedDevoteeName = cleanFilenamePart(s.name, false);
      saveAs(content, `${cleanedDevoteeName}_submission.zip`);
    }
  };
  ...
  const downloadSelectedFiles = async () => {
    ...
    const content = await zip.generateAsync({ type: 'blob' });
    saveAs(content, 'vyas_puja_selected_files.zip');
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        ...
        <div className="flex gap-3 w-full md:w-auto flex-wrap">
          ...
          {userId === 'priyanka' && (
            <button id="btn-download-selected" onClick={downloadSelectedFiles} disabled={selectedIds.length === 0} className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-orange-600 text-white px-5 py-2.5 rounded-xl font-semibold hover:bg-orange-700 transition-all shadow-sm shadow-orange-600/20 text-sm cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">
              <Download className="w-4 h-4" /> Download Selected Files ({selectedIds.length})
            </button>
          )}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-stone-50/80 border-b border-stone-200">
                  {userId === 'priyanka' && (
                    <th className="px-5 py-4 w-10 text-[11px] font-semibold text-stone-500 uppercase tracking-wider">
                      <input
                        type="checkbox"
              ...
              <tbody className="divide-y divide-stone-100">
                {filtered.map((s, idx) => (
                  <tr key={idx} className="hover:bg-orange-50/30 transition-colors">
                    {userId === 'priyanka' && (
                      <td className="px-5 py-4 whitespace-nowrap w-10">
                        <input
                          type="checkbox"
